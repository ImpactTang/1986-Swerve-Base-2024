...
public class SwerveModule extends SubsystemBase {
    ...
    public void setDesiredState(SwerveModuleState desiredState){
        ...
        turnMotor.set(desiredState.angle.getRotations() / Units.radiansToRotations(DriveConstants.kDriveMaxAngularSpeedRadiansPerSecond));
        SmartDashboard.putNumber("Drive Motor Value", driveMotor.getPosition().getValue()); // Update SmartDashboard with Drive Motor Value
        SmartDashboard.putNumber("Turn Motor Value", turnMotor.getPosition().getValue()); // Update SmartDashboard with Turn Motor Value
    ...
    public double getDrivePosition(){
        return driveMotor.getPosition().getValue() * ModuleConstants.driveMotorRot2Meter;
    }

    /* --------------------> Get Drive Motor Velocity as Meters Per Second <-------------------- */
    public double getDriveVelocity(){
        return driveMotor.getVelocity().getValue() * ModuleConstants.driveVelocity2MeterPerSec; 
    }

    /* --------------------> Get Turning Motor Position as Rotations <-------------------- */
    public double getTurningPositionRot(){
        return turnCanCoder.getPosition().getValue();
    }

    /* --------------------> Updating Module CANCoder Values to Smart Dashboard <-------------------- */
    @Override
    public void periodic() {
        SmartDashboard.putNumber(moduleName + "CANcoder Position", turnCanCoder.getPosition().getValue());
    }

    /* --------------------> Resetting Internal Drive Encoders <-------------------- */
    public void resetEncoders(){
        driveMotor.setRotorPosition(0);
        turnMotor.setRotorPosition(turnCanCoder.getPosition().getValue());
    }
